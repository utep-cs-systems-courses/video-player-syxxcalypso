...
def main():
    ...
    video_capture = cv2.VideoCapture(filename)                          #Create VideoCapture obj

    threads = [
        Thread(target=extract_frames, args=[video_capture, maxFrames, color_buffer],), #Func spec Threads
...
def extract_frames(video_capture, maxFrames, color_buffer):
    ...
    while data and data[0] and count <= maxFrames:                      #While there's data & <= cap:
        status, image = data                                            #
        output_buffer.put(image)                                        #Put frame image in queue
    ...
    output_buffer.put(None)                                             #put None to signify done extracting
...
def convert_frames(input_buffer, output_buffer):
    ...
    while not (input_buffer.is_empty() and finished_extracting):        #if buffer !empty& !finished extracting frames

        colored_frame = input_buffer.get()                              #gets colored frame from color_buffer
        if colored_frame is None:                                       #continue if no frames
            continue

        converted_frame = cv2.cvtColor(colored_frame, cv2.COLOR_BGR2GRAY) #converts the color frame to gray
...
def render(input_buffer):
    ...
    while not (input_buffer.is_empty() and finished_filtering):         #graybuffer !empty & !finished filtering
